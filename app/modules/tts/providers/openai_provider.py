...
try:
    ...
    logging.warning("PyAudio not available. Audio streaming functionalities will be limited.")
...
class OpenAITTSProvider(BaseTTSProvider):
    ...
    def __init__(self, redis_client=None, **kwargs):
        ...
        # Simple initialization with latest OpenAI SDK (1.68+)
        try:
            # Only use the API key - all other settings should be compatible
            logger.info("Initializing OpenAI client with version 1.68+")
    ...
    def get_stream_info(self) -> Tuple[int, int, int]:
        ...
        if not PYAUDIO_AVAILABLE:
            logger.warning("PyAudio not available. Returning default audio format values.")
            return (16, 1, self.sample_rate)  # Default values: 16-bit, mono, 24kHz
